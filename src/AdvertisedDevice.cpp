 ...
 * @brief Constructor
 */
AdvertisedDevice::AdvertisedDevice() : m_payload(62, 0) {
  m_advType = 0;
  m_rssi = -9999;
  m_callbackSent = 0;
  m_timestamp = 0;
 ...
 */
uint8_t AdvertisedDevice::getAdvType() {
  return m_advType;
}// getAdvType
 ...
 */
uint8_t AdvertisedDevice::getAdvFlags() {
  size_t data_loc = 0;
 ...
 */
uint16_t AdvertisedDevice::getAppearance() {
  size_t data_loc = 0;
 ...
 */
uint16_t AdvertisedDevice::getAdvInterval() {
  size_t data_loc = 0;
 ...
 */
uint16_t AdvertisedDevice::getMinInterval() {
  size_t data_loc = 0;
 ...
 */
uint16_t AdvertisedDevice::getMaxInterval() {
  size_t data_loc = 0;
 ...
 */
std::string AdvertisedDevice::getManufacturerData(uint8_t index) {
  size_t data_loc = 0;
  index++;
 ...
 */
uint8_t AdvertisedDevice::getManufacturerDataCount() {
 ...
 * @return The URI data.
 */
std::string AdvertisedDevice::getURI() {
  size_t data_loc = 0;
...
*/
std::string AdvertisedDevice::getPayloadByType(uint16_t type) {
  size_t data_loc = 0;
 ...
 */
std::string AdvertisedDevice::getName() {
  size_t data_loc = 0;

  if (findAdvField(BLE_HS_ADV_TYPE_COMP_NAME, 0, &data_loc) > 0 || findAdvField(BLE_HS_ADV_TYPE_INCOMP_NAME, 0, &data_loc) > 0) {
    ble_hs_adv_field *field = (ble_hs_adv_field *) &m_payload[data_loc];
    if (field->length > 1) {
      return std::string((char *) field->value, field->length - 1);
    }
 ...
 */
Scan *AdvertisedDevice::getScan() {
  return Device::getScan();
}// getScan
 ...
 */
uint8_t AdvertisedDevice::getTargetAddressCount() {
  uint8_t count = 0;
 ...
 */
Address AdvertisedDevice::getTargetAddress(uint8_t index) {
  ...
  if (count > 0 && data_loc != ULONG_MAX) {
    ...
    if (field->length > index * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN) {
      return Address(field->value + (index - 1) * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN);
    }
  }
 ...
 */
std::string AdvertisedDevice::getServiceData(uint8_t index) {
  ble_hs_adv_field *field = nullptr;
  uint8_t bytes;
 ...
 * @return The advertised service data or empty string if no data.
 */
std::string AdvertisedDevice::getServiceData(const UUID &uuid) {
  ble_hs_adv_field *field = nullptr;
  uint8_t bytes;
  uint8_t index = 0;
  size_t data_loc = findServiceData(index, &bytes);
  ...
  while (data_loc < plSize) {
    field = (ble_hs_adv_field *) &m_payload[data_loc];
    if (bytes == uuidBytes && UUID(field->value, bytes, false) == uuid) {
      return std::string((char *) (field->value + bytes), field->length - bytes - 1);
    }
 ...
 */
UUID AdvertisedDevice::getServiceDataUUID(uint8_t index) {
  ble_hs_adv_field *field = nullptr;
  uint8_t bytes;
  size_t data_loc = findServiceData(index, &bytes);
  ...
  if (data_loc != ULONG_MAX) {
    field = (ble_hs_adv_field *) &m_payload[data_loc];
    if (field->length >= bytes) {
      return UUID(field->value, bytes, false);
    }
  }
 ...
 */
size_t AdvertisedDevice::findServiceData(uint8_t index, uint8_t *bytes) {
  size_t data_loc = 0;
  uint8_t found = 0;
 ...
 */
uint8_t AdvertisedDevice::getServiceDataCount() {
  uint8_t count = 0;
 ...
 */
UUID AdvertisedDevice::getServiceUUID(uint8_t index) {
  uint8_t count = 0;
  size_t data_loc = 0;
  uint8_t uuidBytes = 0;
  ...
  if (uuidBytes > 0) {
    ...
    if (field->length > uuidBytes * index) {
      return UUID(field->value + uuidBytes * (index - 1), uuidBytes, false);
    }
  }
 ...
 */
uint8_t AdvertisedDevice::getServiceUUIDCount() {
  uint8_t count = 0;
 ...
 */
bool AdvertisedDevice::isAdvertisingService(const UUID &uuid) {
 ...
 */
int8_t AdvertisedDevice::getTXPower() {
  size_t data_loc = 0;
 ...
 */
bool AdvertisedDevice::haveConnParams() {
 ...
 */
bool AdvertisedDevice::haveAdvInterval() {
 ...
 */
bool AdvertisedDevice::haveAppearance() {
 ...
 */
bool AdvertisedDevice::haveManufacturerData() {
...
*/
bool AdvertisedDevice::haveType(uint16_t type) {
 ...
 */
bool AdvertisedDevice::haveTargetAddress() {
 ...
 */
bool AdvertisedDevice::haveName() {
  return findAdvField(BLE_HS_ADV_TYPE_COMP_NAME) > 0 || findAdvField(BLE_HS_ADV_TYPE_INCOMP_NAME) > 0;
}// haveName
 ...
 */
bool AdvertisedDevice::haveRSSI() {
  return m_rssi != -9999;
}// haveRSSI
 ...
 */
bool AdvertisedDevice::haveServiceData() {
 ...
 */
bool AdvertisedDevice::haveServiceUUID() {
 ...
 */
bool AdvertisedDevice::haveTXPower() {
...
uint8_t AdvertisedDevice::findAdvField(uint8_t type, uint8_t index, size_t *data_loc) {
  ble_hs_adv_field *field = nullptr;
 ...
 */
void AdvertisedDevice::setAddress(Address address) {
  m_address = address;
}// setAddress
 ...
 */
void AdvertisedDevice::setAdvType(uint8_t advType, bool isLegacyAdv) {
  m_advType = advType;
#if CONFIG_BT_NIMBLE_EXT_ADV
 ...
 */
void AdvertisedDevice::setRSSI(int rssi) {
  m_rssi = rssi;
}// setRSSI
 ...
 */
std::string AdvertisedDevice::toString() {
  ...
  if (haveManufacturerData()) {
    char *pHex = Utils::buildHexData(nullptr, (uint8_t *) getManufacturerData().data(), getManufacturerData().length());
 ...
 */
uint8_t *AdvertisedDevice::getPayload() {
  return &m_payload[0];
}// getPayload
 ...
 */
void AdvertisedDevice::setPayload(const uint8_t *payload, uint8_t length, bool append) {
  if (!append) {
    m_advLength = length;
 ...
 */
uint8_t AdvertisedDevice::getAdvLength() {
  return m_advLength;
}
 ...
 */
uint8_t AdvertisedDevice::getAddressType() {
 ...
 */
time_t AdvertisedDevice::getTimestamp() {
  return m_timestamp;
}// getTimestamp
 ...
 */
size_t AdvertisedDevice::getPayloadLength() {
 ...
 */
bool AdvertisedDevice::isConnectable() {
#if CONFIG_BT_NIMBLE_EXT_ADV
  if (m_isLegacyAdv) {
 ...
 */
bool AdvertisedDevice::isLegacyAdvertisement() {
#if CONFIG_BT_NIMBLE_EXT_ADV
