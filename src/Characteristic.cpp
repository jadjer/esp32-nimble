...
static CharacteristicCallbacks defaultCallback;
 ...
 */
Characteristic::Characteristic(const char *uuid, uint16_t properties, uint16_t maxLen, Service *pService) : Characteristic(UUID(uuid), properties, maxLen, pService) {
}
 ...
 * @param [in] pService - pointer to the service instance this characteristic belongs to.
 */
Characteristic::Characteristic(const UUID &uuid, uint16_t properties, uint16_t maxLen, Service *pService) : m_value(std::min(CONFIG_NIMBLE_CPP_ATT_VALUE_INIT_LENGTH, (int) maxLen), maxLen) {
  m_uuid = uuid;
...
Characteristic::Characteristic(uint16_t uuid, uint16_t properties, uint16_t maxLen, Service *pService) : Characteristic(UUID(uuid), properties, maxLen, pService) {
}
 ...
 */
Descriptor *Characteristic::createDescriptor(const char *uuid, uint32_t properties, uint16_t max_len) {
  return createDescriptor(UUID(uuid), properties, max_len);
}
 ...
 * @return The new BLE descriptor.
 */
Descriptor *Characteristic::createDescriptor(const UUID &uuid, uint32_t properties, uint16_t max_len) {
  Descriptor *pDescriptor = nullptr;
  if (uuid == UUID(uint16_t(0x2902))) {
    assert(0 && "0x2902 descriptors cannot be manually created");
  } else if (uuid == UUID(uint16_t(0x2904))) {
    pDescriptor = new Descriptor2904(this);
  } else {
    pDescriptor = new Descriptor(uuid, properties, max_len, this);
  }
...
Descriptor *Characteristic::createDescriptor(uint16_t uuid, uint32_t properties, uint16_t max_len) {
 return createDescriptor(UUID(uuid), properties, max_len);
}
 ...
 */
void Characteristic::addDescriptor(Descriptor *pDescriptor) {
  ...
  Device::getServer()->serviceChanged();
}
 ...
 */
void Characteristic::removeDescriptor(Descriptor *pDescriptor, bool deleteDsc) {
 ...
 */
Descriptor *Characteristic::getDescriptorByUUID(const char *uuid) {
  return getDescriptorByUUID(UUID(uuid));
}// getDescriptorByUUID
 ...
 * @return A pointer to the descriptor object or nullptr if not found.
 */
Descriptor *Characteristic::getDescriptorByUUID(const UUID &uuid) {
  for (auto &it : m_dscVec) {
    if (it->getUUID() == uuid) {
      return it;
    }
 ...
 * @return A pointer to the descriptor object or nullptr if not found.
 */
Descriptor *Characteristic::getDescriptorByHandle(uint16_t handle) {
  for (auto &it : m_dscVec) {
 ...
 */
uint16_t Characteristic::getHandle() {
  return m_handle;
}// getHandle
 ...
 */
uint16_t Characteristic::getProperties() {
  return m_properties;
}// getProperties
...
void Characteristic::setService(Service *pService) {
  m_pService = pService;
}
 ...
 */
AttributeValue Characteristic::getValue(time_t *timestamp) {
 ...
 */
size_t Characteristic::getDataLength() {
  return m_value.size();
}
 ...
 */
int Characteristic::handleGapEvent(uint16_t conn_handle, uint16_t attr_handle,
  ...
  Characteristic *pCharacteristic = (Characteristic *) arg;
  ...
  if (ble_uuid_cmp(uuid, &pCharacteristic->getUUID().getNative()->u) == 0) {
    ...
    case BLE_GATT_ACCESS_OP_READ_CHR: {
      ...
      if (ctxt->om->om_pkthdr_len > 8 || pCharacteristic->m_value.size() <= (ble_att_mtu(peerInfo.m_desc.conn_handle) - 3)) {
 ...
 */
size_t Characteristic::getSubscribedCount() const {
 ...
 */
void Characteristic::setSubscribe(struct ble_gap_event *event) {
  ConnectionInfo peerInfo;
  ...
  uint16_t subVal = 0;
  if (event->subscribe.cur_notify > 0 && (m_properties & Property ::NOTIFY)) {
    subVal |= NIMBLE_SUB_NOTIFY;
  }
  if (event->subscribe.cur_indicate && (m_properties & Property::INDICATE)) {
  ...
  if (!event->subscribe.cur_indicate && event->subscribe.prev_indicate) {
    Device::getServer()->clearIndicateWait(event->subscribe.conn_handle);
  }
  ...
  if (subVal > 0) {
    if (it == m_subscribedVec.end()) {
      m_subscribedVec.emplace_back(event->subscribe.conn_handle, subVal);
    } else {
      (*it).second = subVal;
    }
 ...
 */
void Characteristic::indicate(const uint8_t *value, size_t length) {
 ...
 */
void Characteristic::indicate(const std::vector<uint8_t> &value) {
 ...
 */
void Characteristic::notify(bool is_notification, uint16_t conn_handle) {
 ...
 * @param[in] conn_handle Connection handle to send individual notification, or BLE_HCI_LE_CONN_HANDLE_MAX + 1 to send notification to all subscribed clients.
 */
void Characteristic::notify(const std::vector<uint8_t> &value, bool is_notification, uint16_t conn_handle) {
 ...
 * @param[in] conn_handle Connection handle to send individual notification, or BLE_HCI_LE_CONN_HANDLE_MAX + 1 to send notification to all subscribed clients.
 */
void Characteristic::notify(const uint8_t *value, size_t length, bool is_notification, uint16_t conn_handle) {
  NIMBLE_LOGD(LOG_TAG, ">> notify: length: %d", length);

  if (!(m_properties & Property ::NOTIFY) && !(m_properties & Property::INDICATE)) {
    NIMBLE_LOGE(LOG_TAG,
                "<< notify-Error; Notify/indicate not enabled for characteristic: %s",
                std::string(getUUID()).c_str());
  }

  if (m_subscribedVec.empty()) {
  ...
  for (auto &it : m_subscribedVec) {
    ...
    if (!is_notification && (m_properties & Property::INDICATE)) {
      if (!Device::getServer()->setIndicateWait(it.first)) {
      ...
      if (rc != 0) {
        Device::getServer()->clearIndicateWait(it.first);
      }
    } else {
 ...
 */
void Characteristic::setCallbacks(CharacteristicCallbacks *pCallbacks) {
 ...
 */
CharacteristicCallbacks *Characteristic::getCallbacks() {
  return m_pCallbacks;
}//getCallbacks
 ...
 */
void Characteristic::setValue(const uint8_t *data, size_t length) {
 ...
 */
void Characteristic::setValue(const std::vector<uint8_t> &vec) {
 ...
 */
std::string Characteristic::toString() {
 ...
 */
void CharacteristicCallbacks::onRead(Characteristic *pCharacteristic, ConnectionInfo &connInfo) {
 ...
 * @param [in] connInfo A reference to a NimBLEConnInfo instance containing the peer info.
 */
void CharacteristicCallbacks::onWrite(Characteristic *pCharacteristic, ConnectionInfo &connInfo) {
 ...
 * @param [in] pCharacteristic The characteristic that is the source of the event.
 */
void CharacteristicCallbacks::onNotify(Characteristic *pCharacteristic) {
 ...
 */
void CharacteristicCallbacks::onStatus(Characteristic *pCharacteristic, int code) {
 ...
 */
void CharacteristicCallbacks::onSubscribe(Characteristic *pCharacteristic,
