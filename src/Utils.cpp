 ...
 */
int Utils::checkConnParams(ble_gap_conn_params* params) {
 ...
 */
const char*Utils::returnCodeToString(int rc) {
 ...
 */
const char*Utils::advTypeToString(uint8_t advType) {
 ...
 */
char*Utils::buildHexData(uint8_t* target, const uint8_t* source, uint8_t length) {
 ...
 * @param [in] arg Unused.
 */
void Utils::dumpGapEvent(ble_gap_event *event, void *arg){
    (void)arg;
 ...
 */
const char*Utils::gapEventToString(uint8_t eventType) {
#if defined(CONFIG_NIMBLE_CPP_ENABLE_GAP_EVENT_CODE_TEXT)
    switch (eventType) {
