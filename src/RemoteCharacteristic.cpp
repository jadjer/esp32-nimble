 ...
 *  };
 */
RemoteCharacteristic::RemoteCharacteristic(RemoteService *pRemoteService,
                                           const struct ble_gatt_chr *chr) {
  NIMBLE_LOGD(LOG_TAG, ">> NimBLERemoteCharacteristic()");
  switch (chr->uuid.u.type) {
  case BLE_UUID_TYPE_16:
    m_uuid = UUID(chr->uuid.u16.value);
    break;
  case BLE_UUID_TYPE_32:
    m_uuid = UUID(chr->uuid.u32.value);
    break;
  case BLE_UUID_TYPE_128:
    m_uuid = UUID(const_cast<ble_uuid128_t *>(&chr->uuid.u128));
    break;
  default:
    break;
  }
 ...
 */
bool RemoteCharacteristic::canBroadcast() {
 ...
 */
bool RemoteCharacteristic::canIndicate() {
 ...
 */
bool RemoteCharacteristic::canNotify() {
 ...
 */
bool RemoteCharacteristic::canRead() {
 ...
 */
bool RemoteCharacteristic::canWrite() {
 ...
 */
bool RemoteCharacteristic::canWriteNoResponse() {
 ...
 */
int RemoteCharacteristic::descriptorDiscCB(uint16_t conn_handle,
  ...
  const UUID *uuid_filter = filter->uuid;
  ble_task_data_t *pTaskData = (ble_task_data_t *) filter->task_data;
  RemoteCharacteristic *characteristic = (RemoteCharacteristic *) pTaskData->pATT;
  ...
  case 0: {
    ...
    RemoteDescriptor *pNewRemoteDescriptor = new RemoteDescriptor(characteristic, dsc);
 ...
 */
int RemoteCharacteristic::nextCharCB(uint16_t conn_handle,
                                     const struct ble_gatt_error *error,
  ...
  RemoteCharacteristic *pChar = (RemoteCharacteristic *) pTaskData->pATT;
 ...
 */
bool RemoteCharacteristic::retrieveDescriptors(const UUID *uuid_filter) {
  ...
  if (m_endHandle == 0) {
    rc = ble_gattc_disc_all_chrs(getRemoteService()->getClient()->getConnId(),
                                 ...
                                 RemoteCharacteristic::nextCharCB,
                                 &taskData);
    if (rc != 0) {
  ...
  rc = ble_gattc_disc_all_dscs(getRemoteService()->getClient()->getConnId(),
                               m_handle,
                               m_endHandle,
                               RemoteCharacteristic::descriptorDiscCB,
                               &filter);

  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gattc_disc_all_dscs: rc=%d %s", rc, Utils::returnCodeToString(rc));
    return false;
  }
 ...
 */
RemoteDescriptor *RemoteCharacteristic::getDescriptor(const UUID &uuid) {
 ...
 */
std::vector<RemoteDescriptor *> *RemoteCharacteristic::getDescriptors(bool refresh) {
  if (refresh) {
    deleteDescriptors();
 ...
 */
std::vector<RemoteDescriptor *>::iterator RemoteCharacteristic::begin() {
 ...
 */
std::vector<RemoteDescriptor *>::iterator RemoteCharacteristic::end() {
 ...
 */
uint16_t RemoteCharacteristic::getHandle() {
  return m_handle;
}// getHandle
 ...
 */
uint16_t RemoteCharacteristic::getDefHandle() {
  return m_defHandle;
}// getDefHandle
 ...
 */
RemoteService *RemoteCharacteristic::getRemoteService() {
 ...
 */
UUID RemoteCharacteristic::getUUID() {
  return m_uuid;
}// getUUID
 ...
 */
AttributeValue RemoteCharacteristic::getValue(time_t *timestamp) {
 ...
 * @return The value of the remote characteristic.
 */
AttributeValue RemoteCharacteristic::readValue(time_t *timestamp) {
  NIMBLE_LOGD(LOG_TAG, ">> readValue(): uuid: %s, handle: %d 0x%.2x",
              getUUID().toString().c_str(), getHandle(), getHandle());

  Client *pClient = getRemoteService()->getClient();
 ...
 */
int RemoteCharacteristic::onReadCB(uint16_t conn_handle,
                                   const struct ble_gatt_error *error,
 ...
 */
bool RemoteCharacteristic::setNotify(uint16_t val, notify_callback notifyCallback, bool response) {
  NIMBLE_LOGD(LOG_TAG, ">> setNotify(): %s, %02x", toString().c_str(), val);

  m_notifyCallback = notifyCallback;

  RemoteDescriptor *desc = getDescriptor(UUID((uint16_t) 0x2902));
  if (desc == nullptr) {
 ...
 * @return false if writing to the descriptor failed.
 */
bool RemoteCharacteristic::subscribe(bool notifications, notify_callback notifyCallback, bool response) {
  if (notifications) {
 ...
 * @return false if writing to the descriptor failed.
 */
bool RemoteCharacteristic::unsubscribe(bool response) {
 ...
 */
void RemoteCharacteristic::deleteDescriptors() {
 ...
 */
size_t RemoteCharacteristic::deleteDescriptor(const UUID &uuid) {
 ...
 */
std::string RemoteCharacteristic::toString() {
 ...
 */
bool RemoteCharacteristic::writeValue(const std::vector<uint8_t> &vec, bool response) {
 ...
 * @return false if not connected or otherwise cannot perform write.
 */
bool RemoteCharacteristic::writeValue(const char *char_s, bool response) {
 ...
 * @return false if not connected or otherwise cannot perform write.
 */
bool RemoteCharacteristic::writeValue(const uint8_t *data, size_t length, bool response) {
  ...
  do {
    if (length > mtu) {
      ...
      rc = ble_gattc_write_long(pClient->getConnId(), m_handle, 0, om,
                                RemoteCharacteristic::onWriteCB,
                                &taskData);
    } else {
 ...
 * @return success == 0 or error code.
 */
int RemoteCharacteristic::onWriteCB(uint16_t conn_handle,
                                    const struct ble_gatt_error *error,
                                    struct ble_gatt_attr *attr, void *arg) {
  ble_task_data_t *pTaskData = (ble_task_data_t *) arg;
  RemoteCharacteristic *characteristic = (RemoteCharacteristic *) pTaskData->pATT;
