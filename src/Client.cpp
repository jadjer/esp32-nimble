 ...
 */
Client::Client(const Address &peerAddress) : m_peerAddress(peerAddress) {
 ...
 */
void Client::dcTimerCb(ble_npl_event *event) {
 ...
 */
size_t Client::deleteService(const UUID &uuid) {
 ...
 */
bool Client::connect(bool deleteAttributes) {
 ...
 * @return True on success.
 */
bool Client::connect(AdvertisedDevice *device, bool deleteAttributes) {
...
bool Client::connect(const Address &address, bool deleteAttributes) {
  ...
  if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(m_connectTimeout + 1000)) == pdFALSE) {
    ...
    NIMBLE_LOGE(LOG_TAG, "Connection failed; status=%d %s",
                taskData.rc,
                Utils::returnCodeToString(taskData.rc));
...
bool Client::secureConnection() {
  ...
  do {
    ...
    int rc = Device::startSecurity(m_conn_id);
 ...
 */
int Client::disconnect(uint8_t reason) {
 ...
 */
void Client::setConnectionParams(uint16_t minInterval, uint16_t maxInterval,
                                ...
                                uint16_t minConnTime, uint16_t maxConnTime)*/
{
  ...
  int rc = Utils::checkConnParams(&m_pConnParams);
 ...
 * @param [in] timeout The timeout time in 10ms units before disconnecting.
 */
void Client::updateConnParams(uint16_t minInterval, uint16_t maxInterval,
                              uint16_t latency, uint16_t timeout) {
 ...
 */
void Client::setDataLen(uint16_t tx_octets) {
 ...
 */
ConnectionInfo Client::getConnInfo() {
 ...
 */
void Client::setConnectTimeout(uint32_t time) {
 ...
 */
void Client::setPeerAddress(const Address &address) {
  if (isConnected()) {
...
int Client::getRssi() {
  ...
  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "Failed to read RSSI error code: %d, %s",rc, Utils::returnCodeToString(rc));
    m_lastErr = rc;
    return 0;
  }
 ...
 */
std::vector<RemoteService *>::iterator Client::begin() {
 ...
 */
std::vector<RemoteService *>::iterator Client::end() {
 ...
 */
RemoteService *Client::getService(const char *uuid) {
  return getService(UUID(uuid));
}// getService
 ...
 * @return A pointer to the service or nullptr if not found.
 */
RemoteService *Client::getService(const UUID &uuid) {
 ...
 */
std::vector<RemoteService *> *Client::getServices(bool refresh) {
  if (refresh) {
    deleteServices();
 ...
 */
bool Client::discoverAttributes() {
  deleteServices();
 ...
 */
bool Client::retrieveServices(const UUID *uuid_filter) {
  /**
 * Design
 * ------
 ...
 */
  ...
  if (uuid_filter == nullptr) {
    rc = ble_gattc_disc_all_svcs(m_conn_id, Client::serviceDiscoveredCB, &taskData);
  } else {
    rc = ble_gattc_disc_svc_by_uuid(m_conn_id, &uuid_filter->getNative()->u,
                                    Client::serviceDiscoveredCB, &taskData);
  }

  if (rc != 0) {
    NIMBLE_LOGE(LOG_TAG, "ble_gattc_disc_all_svcs: rc=%d %s", rc, Utils::returnCodeToString(rc));
    m_lastErr = rc;
    return false;
  }
 ...
 */
int Client::serviceDiscoveredCB(
  ...
  Client *client = (Client *) pTaskData->pATT;
  ...
  if (error->status == 0) {
    ...
    RemoteService *pRemoteService = new RemoteService(client, service);
  ...
  if (error->status == BLE_HS_EDONE) {
    ...
    NIMBLE_LOGE(LOG_TAG, "serviceDiscoveredCB() rc=%d %s",error->status, Utils::returnCodeToString(error->status));
 ...
 */
AttributeValue Client::getValue(const UUID &serviceUUID, const UUID &characteristicUUID) {
  ...
  AttributeValue ret;
  RemoteService *pService = getService(serviceUUID);

  if (pService != nullptr) {
    RemoteCharacteristic *pChar = pService->getCharacteristic(characteristicUUID);
    if (pChar != nullptr) {
      ret = pChar->readValue();
    }
  }
 ...
 */
bool Client::setValue(const UUID &serviceUUID, const UUID &characteristicUUID,
                      const AttributeValue &value, bool response) {
 ...
 */
RemoteCharacteristic *Client::getCharacteristic(const uint16_t handle) {
  RemoteService *pService = nullptr;
  for (auto & it : m_servicesVector) {
 ...
 */
/*STATIC*/
int Client::handleGapEvent(struct ble_gap_event *event, void *arg) {
  Client *pClient = (Client *) arg;
  int rc;

  NIMBLE_LOGD(LOG_TAG, "Got Client event %s", Utils::gapEventToString(event->type));
  ...
  case BLE_GAP_EVENT_DISCONNECT: {
    ...
    Device::removeIgnored(pClient->m_peerAddress);
  ...
  case BLE_GAP_EVENT_CONNECT: {
    ...
    if (rc == 0) {
      ...
      rc = ble_gattc_exchange_mtu(pClient->m_conn_id, nullptr, nullptr);
      if (rc != 0) {
      ...
      Device::addIgnored(pClient->m_peerAddress);
    } else {
      pClient->m_conn_id = BLE_HS_CONN_HANDLE_NONE;
      break;
    }
      ...
      if (characteristic != cVector->cend()) {
        ...
        if ((*characteristic)->m_notifyCallback != nullptr) {
          NIMBLE_LOGD(LOG_TAG, "Invoking callback for notification on characteristic %s",
                      (*characteristic)->toString().c_str());
          (*characteristic)->m_notifyCallback(*characteristic, event->notify_rx.om->om_data, data_len, !event->notify_rx.indication);
        }
        break;
      }
    }
  ...
  case BLE_GAP_EVENT_PASSKEY_ACTION: {
    ...
    if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
      ...
      pkey.passkey = Device::m_passkey;// This is the passkey to be entered on peer
 ...
 */
void Client::setClientCallbacks(ClientCallbacks *pClientCallbacks, bool deleteCallbacks) {
 ...
 */
int Client::getLastError() {
  return m_lastErr;
}// getLastError

void ClientCallbacks::onConnect(Client *pClient) {
  NIMBLE_LOGD("NimBLEClientCallbacks", "onConnect: default");
}

void ClientCallbacks::onDisconnect(Client *pClient, int reason) {
  NIMBLE_LOGD("NimBLEClientCallbacks", "onDisconnect: default");
}

bool ClientCallbacks::onConnParamsUpdateRequest(Client *pClient, const ble_gap_upd_params *params) {
...
void ClientCallbacks::onAuthenticationComplete(ConnectionInfo &peerInfo) {
